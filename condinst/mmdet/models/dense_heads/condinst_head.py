...
def glrdn_l1(tb, yb):
    ...
    tl = tb[:, 1:h-1, 0:w-2]
    tr = tb[:, 1:h-1, 2:w]
    reg_sum = ((torch.abs((t0-tu) - (y0 - yu))).sum() +
               (torch.abs((t0-td) - (y0 - yd))).sum() +
               (torch.abs((t0-tl) - (y0 - yl))).sum() +
               (torch.abs((t0-tr) - (y0 - yr))).sum())
...
class CondInstHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_centerness=dict(
                     ...
                     use_sigmoid=True,
                     loss_weight=1.0),
                 # select from ['glrdn-l2', 'glrdn-l1', 'glr-l2', 'None']
                 loss_reg=dict(func='None', loss_weight=0.0, warmup=0),
                 **kwargs):
        ...
        self.reg_loss_func = loss_reg['func']
        self.reg_loss_weight = loss_reg['loss_weight']
        self.reg_loss_warmup = loss_reg['warmup']
        self.glreg = without_glr
        if (self.reg_loss_func == "glrdn-l2"):
            self.glreg = glrdn_l2
            print("set Graph Laplacian Regularizer : GLRDN_L2")
        elif (self.reg_loss_func == "glrdn-l1"):
            self.glreg = glrdn_l1
            print("set Graph Laplacian Regularizer : GLRDN_L1")
        elif (self.reg_loss_func == "glr-l2"):
            self.glreg = glr_l2
            print("set Graph Laplacian Regularizer : GLR_L2")
        else:
            print("set Graph Laplacian Regularizer : WITHOUT")
    ...
    def loss(self,
        ...
        for i in range(num_imgs):
            ...
            loss_glrdn += self.glreg(mask_logits.sigmoid(), gt_mask.float())
            num_mask += len(idx_gt)

        loss_mask = loss_mask / num_mask
        if (self.reg_loss_warmup < self.reg_warmup_count):
            res_enable = 1.0
        else:
            res_enable = 0.0
        loss_glrdn = loss_glrdn * self.reg_loss_weight * res_enable
        self.reg_warmup_count += 1
